...
<p>"${greeting_text}, ${last_name} ${first_name} ${second_name}"<p>



<form action="/cars" method="POST">
    <input type="hidden"  name="acc_type" value="manager" />
    <input type="hidden"  name="req_type" value="unconfirmed" />
    <input type="submit" value="${contract_unconfirmed_text}" />
</form>

<form action="/cars" method="POST">
    <input type="hidden"  name="acc_type" value="manager" />
    <input type="hidden"  name="req_type" value="rented" />
...
<table id="cars">
    ...
    <c:forEach varStatus="i" var="car" items="${cars}" >
        <tr class="car">
            ...
            <td><p>${car.price}</p></td>
            <td>
                <c:if test = "${car.contract_state == 'free'}">
                    <p>${rent_avaible_text}</p>
                </c:if>

                <c:if test = "${car.contract_state == 'unconfirmed'}">
                    <p>${contract_unconfirmed_text}</p>

                    <form action="/update_rent_manager" method="POST">
                        <input type="hidden"  name="acc_type" value="manager" />
                        <input type="hidden"  name="req_type" value="confirm" />
                        <input type="hidden"  name="car_id" value="${car.id}" />
                        <input type="submit" value="${confirm_text}" />
                    </form>
                    <form action="/update_rent_manager" method="POST">
                        <input type="hidden"  name="acc_type" value="manager" />
                        <input type="hidden"  name="req_type" value="cancel" />
                        <input type="hidden"  name="car_id" value="${car.id}" />
                        <input type="submit" value="${cancel_text}" />
                    </form>
                </c:if>

                <c:if test = "${car.contract_state == 'confirmed'}">
                    <p>${contract_confirmed_text}</p>

                    <form action="/update_rent_manager" method="POST">
                        <input type="hidden"  name="acc_type" value="manager" />
                        <input type="hidden"  name="req_type" value="close" />
                        <input type="hidden"  name="car_id" value="${car.id}" />
                        <input type="submit" value="${close_text}" />
                    </form>
                </c:if>

                <c:if test = "${car.contract_state == 'finished'}">
                    <p>${contract_finished_text_text}</p>
                </c:if>

                <c:if test = "${car.contract_state == 'canceled'}">
                    <p>${contract_canceled_text_text}</p>
                </c:if>
